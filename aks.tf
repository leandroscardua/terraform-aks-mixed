resource "azurerm_resource_group" "aks" {
  name     = "aks-${random_string.aks.id}-rg"
...
resource "azurerm_kubernetes_cluster" "aks" {
  name                          = "aks-${random_string.aks.id}-cluster"
  kubernetes_version            = var.orchestrator_version
  location                      = azurerm_resource_group.aks.location
  resource_group_name           = azurerm_resource_group.aks.name
  dns_prefix                    = "aks-${random_string.aks.id}"
  sku_tier                      = "Free"
...
resource "null_resource" "deploy_app" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      kubectl apply -f https://raw.githubusercontent.com/leandroscardua/windows-linux-aks-sample/main/app.yml
